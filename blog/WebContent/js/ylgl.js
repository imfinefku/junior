...
function initTable() {
	table.render({
		elem : '#grid',
		url : 'youLianController.do?method=getData',
		toolbar : '#toolbar',
		id : "grid",
...
function initForm() {
	...
	// 提交表单
	form.on('submit(submitYouLian)', function(data) {
		layer.close(curOpen);
		if (curType == "insert") {
			insertYouLian(data.field);
		} else if (curType == "update") {
			updateYouLian(data.field);
		}
		return false;
	});
}
...
function delYouLian(id) {
	$.ajax({
		url : "youLianController.do?method=delete",
		dataType : "json",
		data : {
			"id" : id
		},
...
// 添加友链
function insertYouLian(data) {
	$.ajax({
		url : "youLianController.do?method=insert",
		dataType : "json",
		data : {
			"name" : data.name,
			"url":data.url,
...
// 修改友链
function updateYouLian(data) {
	$.ajax({
		url : "youLianController.do?method=update",
		dataType : "json",
		data : {
			"name" : data.name,
			"url":data.url,
